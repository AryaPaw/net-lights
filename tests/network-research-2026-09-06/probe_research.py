...
TARGETS = [
    ...
    ('google', 'Мир', 'google', 'https://connectivitycheck.gstatic.com/generate_204', 'GET', 204, ''),
    ('cloudflare', 'Мир', 'cloudflare', 'https://cp.cloudflare.com/generate_204', 'GET', 204, ''),
    ('mozilla', 'Мир', 'fastly', 'https://firefox-portal-detection.com/generate_204', 'GET', 204, ''),
    ('fedora', 'Мир', 'unc', 'https://fedoraproject.org/static/hotspot.txt', 'GET', 200, 'OK'),
    ('apple', 'Мир', 'akamai', 'https://www.apple.com/library/test/success.html', 'GET', 200, '<HTML><HEAD><TITLE>Success</TITLE></HEAD><BODY>Success</BODY></HTML>'),
    ('kde', 'Мир', 'fastly', 'https://networkcheck.kde.org/', 'GET', 200, 'OK'),
    ('debian', 'Мир', 'utwente', 'https://www.debian.org/', 'HEAD', 200, None),
    ('arch', 'Мир', 'haproxy', 'https://archlinux.org/', 'HEAD', 200, None),
    ('github', 'Мир', 'github', 'https://github.com/robots.txt', 'HEAD', 200, None),
    ('wikimedia', 'Мир', 'wikimedia', 'https://en.wikipedia.org/robots.txt', 'HEAD', 200, None),
]
...
def main():
    ...
    if any(not any(t[1] == g for t in targets_selected) for g in ('RU','Мир')):
    ...
    seq = 0
    groups = {g: [t for t in targets_selected if t[1] == g] for g in ('RU', 'Мир')}
    cursor = {'RU': 0, 'Мир': 0}
