...
public sealed class SchedulerTests
{
    ...
    public void T01_StartWithoutResults_IsUnknown()
    {
        ...
        Assert.Equal(GroupAvailability.Unknown, runner.Kernel.Snapshot.World.Availability);
        runner.Run(TimeSpan.FromSeconds(6));
        Assert.Equal(GroupAvailability.Online, runner.Kernel.Snapshot.Ru.Availability);
        Assert.Equal(GroupAvailability.Online, runner.Kernel.Snapshot.World.Availability);
    }
    ...
    public void T04_TwoLive_IsOnline(int a, int b)
    {
        ...
        runner.RestoreOnly(live.Concat(Enumerable.Range(0, 7).Select(i => $"world-{i}")).ToArray());
    ...
    {
        var runner = new VirtualRunner();
        runner.RestoreOnly("ru-3", "world-0", "world-1", "world-2", "world-3", "world-4", "world-5", "world-6");
        runner.Run(TimeSpan.FromSeconds(45), TimeSpan.FromMilliseconds(200));
    ...
    public void OutageAfterOnline_DoesNotStayLimitedOnStaleSuccess()
    {
        ...
        Assert.Equal(GroupAvailability.Offline, runner.Kernel.Snapshot.World.Availability);
    }
    ...
    public void T24_DualOutageWithin14s()
    {
        foreach (int phaseMs in Enumerable.Range(0, 280).Select(i => i * 50))
        {
            ...
            Assert.True(
                ...
                && runner.Kernel.Snapshot.World.Availability == GroupAvailability.Offline,
                $"phase {phaseMs} ru={runner.Kernel.Snapshot.Ru.Availability} world={runner.Kernel.Snapshot.World.Availability} elapsed={(runner.Time.GetUtcNow() - start).TotalSeconds}");
            Assert.InRange(runner.MaxInflight, 0, 4);
        }
    }
    ...
    public void T25_PartialRecoveryWithin15s(int liveIndex)
    {
        ...
        runner.BreakAll();
        runner.Run(TimeSpan.FromSeconds(20), TimeSpan.FromMilliseconds(50));
        runner.RestoreOnly($"ru-{liveIndex}", "world-0", "world-1");
        DateTimeOffset start = runner.Time.GetUtcNow();
        for (int i = 0; i < 300; i++)
        {
            runner.Run(TimeSpan.FromMilliseconds(50));
    ...
    private static void AssertEqualRotation(VirtualRunner runner)
    {
        ...
        int world = runner.StartLog.Count(id => id.StartsWith("world-", StringComparison.Ordinal));
        Assert.InRange(Math.Abs(ru - world), 0, 2);
    }
}
