...
internal sealed class NetLightsContext : ApplicationContext
{
    ...
    private GroupAvailability _historyWorld;
    ...
    public NetLightsContext()
    {
        ...
        _historyWorld = _snapshot.World.Availability;
        ...
        _icon = new NotifyIcon
        {
            ...
            Icon = _renderer.Get(_snapshot.Ru.Availability, _snapshot.World.Availability, iconSize)
        };
    ...
    private void ApplySnapshot(MonitorSnapshot snapshot)
    {
        _snapshot = snapshot;
        int iconSize = _renderer.SystemSmallIconSize();
        Icon icon = _renderer.Get(snapshot.Ru.Availability, snapshot.World.Availability, iconSize);
        ...
        if (snapshot.Ru.Availability != _historyRu || snapshot.World.Availability != _historyWorld)
        {
            _historyRu = snapshot.Ru.Availability;
            _historyWorld = snapshot.World.Availability;
            try
            {
                StateHistoryStore.Append(snapshot.GeneratedUtc, snapshot.Ru.Availability, snapshot.World.Availability);
            }
            catch (Exception ex)
            {
    ...
    private void CheckSnapshotAge()
    {
        ...
        {
            _icon.Text = "Провайдер: нет свежих данных | Мир: нет свежих данных";
        }
    }
