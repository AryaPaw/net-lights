...
internal static class StateHistoryStore
{
    ...
    public static void Append(DateTimeOffset utc, GroupAvailability ru, GroupAvailability world)
    {
        Directory.CreateDirectory(SettingsStore.RootDirectory);
        var record = new HistoryRecord(utc, ru.ToString(), world.ToString());
    ...
    private sealed record HistoryRecord(DateTimeOffset Utc, string Ru, string World);
}
