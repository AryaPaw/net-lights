...
internal sealed class StatusForm : Form
{
    ...
    public void Bind(MonitorSnapshot snapshot)
    {
        ...
        string title = $"Net Lights - Провайдер: {DiagnosticExport.Label(snapshot.Ru.Availability)}, Мир: {DiagnosticExport.Label(snapshot.World.Availability)}";
        string summary = $"Провайдер: {DiagnosticExport.Label(snapshot.Ru.Availability)}  |  Мир: {DiagnosticExport.Label(snapshot.World.Availability)}";
        if (snapshot.Ru.ConfirmationActive || snapshot.World.ConfirmationActive)
        {
    ...
    private static List<EndpointRow> Rows(MonitorSnapshot snapshot)
    {
        ...
        AddGroup(rows, "Мир", snapshot.World);
        return rows;
    }
