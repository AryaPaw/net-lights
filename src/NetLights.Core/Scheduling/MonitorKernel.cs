...
public sealed partial class MonitorKernel
{
    ...
    private readonly GroupRuntime _world;
    ...
    public MonitorKernel(MonitorConfiguration config, TimeProvider time)
    {
        ...
        BeginConfirmation(_world, _clock.Now, "startup");
    ...
    public int PhysicalInflight => _ru.PhysicalCount + _world.PhysicalCount;
    ...
    public IReadOnlyList<WorkItem> BeginNewEpoch(string reason)
    {
        ...
        ResetGroupEvidence(_world);
        BeginConfirmation(_ru, _clock.Now, reason);
        BeginConfirmation(_world, _clock.Now, reason);
    ...
    {
        foreach (EndpointSlot slot in _ru.Slots.Concat(_world.Slots))
        {
